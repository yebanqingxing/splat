	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="company" name="office.id" value="${not empty user.id?user.office.id:''}"  labelName="officeName" labelValue="${not empty user.id?user.office.name:''}" notAllowSelectRoot="false"
					title="所属部门" url="/sys/office/treeData?platType=S&isAll=true&type=1" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="role" name="roleIdList" value="${not empty user.id?user.roleIds:''}"  labelName="role.name" labelValue="${not empty user.id?user.roleNames:''}"
					isAddUser="true" title="角色" url="/sys/role/treeDate?platType=S" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${empty user.id }">
			...
			<div class="control-group">
				...
				<div class="controls">
					<input id="confirmNewPassword" name = "confimpassword" type="password" value="" maxlength="50" minlength="3" class="required" equalTo="#newPassword"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</c:if>
