...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/sys/user/supplierList");
			$("#searchForm").submit();
	    	return false;
	    }
...
<body>
	...
	<form:form id="searchForm" modelAttribute="user" action="${ctx}/sys/user/supplierList" method="post" class="breadcrumb form-search" style="background-color:#FFF;">
		...
		<ul class="ul-form">
			...
			<li>
				...
				<sys:treeselect id="office" name="office.id" value="${user.office.id}" 
								...
								title="部门" url="/sys/office/treeData?platType=S&type=2" cssClass="input-small" allowClear="true" notAllowSelectParent="false"/>
			</li> 
		...
		<c:forEach items="${page.list}" var="user">
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit">
					<td>
						<a href="${ctx}/sys/user/detial?platType=S&id=${user.id}" class="btn btn-xs red">详情 <i class="fa fa-edit"></i></a>
						<a href="${ctx}/sys/user/supplierForm?id=${user.id}" class="btn btn-xs red">修改 <i class="fa fa-edit"></i></a>
						<a onclick=resetPassword("${user.id}")  class="btn btn-xs red">
							重置密码 
							<i class="fa fa-edit"></i>
						</a>
	    				<a href="${ctx}/sys/user/delete?platType=S&id=${user.id}" onclick="return confirmx('确认要删除该用户吗？', this.href)" class="btn btn-xs purple">删除 <i class="fa fa-times"></i></a>
					</td>
				</shiro:hasPermission>
			</tr>
		</c:forEach>
