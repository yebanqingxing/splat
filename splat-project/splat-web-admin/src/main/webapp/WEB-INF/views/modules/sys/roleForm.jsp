...
<body>
		<div class="effect-head clear" style="margin-top:31px;margin-bottom:4px;">
			<p>操作员角色<shiro:hasPermission name="sys:role:edit">${not empty role.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:role:edit">查看</shiro:lacksPermission>：</p>
			<img src="${ctxStatic}/images/right.jpg">
		</div>
		<div class="clear"></div>
	<form:form id="inputForm" modelAttribute="role" action="${ctx}/sys/role/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		<input type = "hidden" name="platType" value = "A" />
		...
		<%-- <div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="office" name="office.id" value="${role.office.id}" labelName="office.name" labelValue="${role.office.name}"
					title="机构" url="/sys/office/treeData?platType=A&isAll=true" cssClass="required"/>
			</div>
		</div> --%>
		<br>
		<div class="control-group">
		...
		<%-- <div class = "control-group">
			...
			<form:select path="platType" class="controls">
				<form:option value = "A" selected="${role.platType == 'A'?'':true }">平台</form:option>
				<form:option value = "S" selected="${role.platType == 'S'?'':true }">供应商</form:option>
				<form:option value = "B" selected="${role.platType == 'B'?'':true }">采购商</form:option>
			</form:select>
		</div> --%>
		<div class="control-group">
		...
		<div class="form-actions">
			<%-- <c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
				<shiro:hasPermission name="sys:role:edit"> --%><input id="btnSubmit" class="btn green" type="submit" value="保 存"/>&nbsp;
				<%--</shiro:hasPermission>
			 </c:if> --%>
			<a href = "${ctx}/sys/role/list"><input id="btnCancel" class="btn default" type="button" value="返 回" /></a>
		</div>
	</form:form>
	<script>
		/* $(document).ready(function(){
			...
			//console.log(parent.window.frames[iframId].document.height);
			/* parent.window.frames[iframId].height=300;
			function iFrameHeight() {
				var ifm= parent.window.frames[iframId];
				var subWeb = document.frames ? document.ifm.document :
				ifm.contentDocument;
