...
<body>
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<input type = "hidden" id = "isAdmin" value = "${fns:getUser().admin}"/>
		<form:hidden path="platType" value = "A"/>
		<sys:message content="${message}"/>
		<input id = "office_id" type = "hidden" value = ""/>
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="company" name="office.id" value="${not empty user.id?user.office.id:''}"  labelName="officeName" labelValue="${not empty user.id?user.office.name:''}" notAllowSelectRoot="false"
					 isAll="true" title="所属部门" url="/sys/office/treeData?platType=A" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="role" name="roleIdList" value="${not empty user.id?user.roleIds:''}"  labelName="roleName" labelValue="${not empty user.id?user.roleNames:''}" 
					isAddUser="true" title="角色" url="/sys/role/treeDate?platType=A" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test = "${empty user.id }">
			...
			<div class="control-group">
				...
				<div class="controls">
					<input name = "confirmpassword" type="password"  maxlength="50" minlength="3" equalTo="#newPassword" class="required"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</c:if>
		...
		<%-- <div class = "control-group">
			...
			<form:select path="platType" class="controls">
				<form:option value = "A" selected="${user.platType == 'A'?'':true }">平台</form:option>
				<form:option value = "S" selected="${user.platType == 'S'?'':true }">供应商</form:option>
				<form:option value = "B" selected="${user.platType == 'B'?'':true }">采购商</form:option>
			</form:select>
		</div> --%>
		<%-- <div class="control-group">
