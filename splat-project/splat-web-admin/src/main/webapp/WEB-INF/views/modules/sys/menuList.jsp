			...
			<c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne '1'?menu.parent.id:'0'}">
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a href="${ctx}/sys/menu/form?id=${menu.id}" class="btn btn-xs red">修改  <i class="fa fa-edit"></i></a>
						<a href="${ctx}/sys/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)" class="btn btn-xs purple">删除  <i class="fa fa-times"></i></a>
						<a href="${ctx}/sys/menu/form?parent.id=${menu.id}" class="btn btn-xs green">添加下级菜单  <i class="fa fa-plus"></i></a>
