...
<head>
	...
	<script type="text/javascript">
		...
		function goAddOffice(){
			$("#searchForm").attr("action","${ctx}/sys/office/supplierForm");
			$("#searchForm").submit();
		}
	</script>
</head>
<body>
	...
	<form:form id="searchForm" modelAttribute="office" action="${ctx}/sys/office/supplierList?type=1" method="post" class="form-horizontal">
		...
		<div class="query clear">
			<span class="query-span" style="width: 90px; display: inline-block;">供应商名称：</span>
		...
		<div class="query">
			...
			<button class="addquery" style="width:80px;" onclick="goAddOffice()">添加供应商</button>
				...
				<shiro:hasPermission name="sys:user:edit">
					<td>
						...
						<c:if test="${office.count==0}">
							<a href="${ctx}/sys/user/supplierForm" class="btn btn-xs red">创建登陆用户 <i class="fa fa-edit"></i></a>
						</c:if>
						<c:if test="${office.count>0}">
							<a href="${ctx}/sys/user/supplierList" class="btn btn-xs red">查看登陆用户 <i class="fa fa-edit"></i></a>
	    				</c:if>
