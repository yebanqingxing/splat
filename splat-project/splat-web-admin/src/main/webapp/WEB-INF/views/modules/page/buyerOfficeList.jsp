...
<head>
	...
	<script type="text/javascript">
		...
		function goAddOffice(){
			$("#searchForm").attr("action","${ctx}/sys/office/buyerForm");
			$("#searchForm").submit();
		}
	</script>
</head>
...
<div class="effect-head clear" style="margin-top:20px;">
	...
	<form:form id="searchForm" modelAttribute="office" action="${ctx}/sys/office/buyerList?type=1" method="post" class="form-horizontal">
		...
		<div class="query">
			...
			<button class="addquery" type="submit" onclick="return page()">查询</button>
			<button class="addquery" style="width:80px;" onclick="goAddOffice()">添加采购商</button>
			...
			<tr>
				...
				<shiro:hasPermission name="sys:user:edit">
					<td>
						...
						<c:if test="${office.count==0}">
							<a href="${ctx}/sys/user/buyerForm" class="btn btn-xs red" >创建登陆用户 <i class="fa fa-edit"></i></a>
						</c:if>
	    				<c:if test="${office.count>0}">
	    					<a href="${ctx}/sys/user/buyerList" class="btn btn-xs red" >查看登陆用户 <i class="fa fa-edit"></i></a>
	    				</c:if>
