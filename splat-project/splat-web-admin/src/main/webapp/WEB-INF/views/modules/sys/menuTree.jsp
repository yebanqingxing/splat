		...
		<c:if test="${menu.parent.id eq (not empty param.parentId ? param.parentId:1)&&menu.isShow eq '1'}">
			<div class="nav_add">
				...
				<c:forEach items = "${menuList }" var = "menu2">
					...
					<c:if test="${flag != 1 }">
					<c:choose>
						<c:when test="${menu.name eq '供应商维护'}">
							<p class="nav_addclick"><img src="${ctxStatic}/images/nav_add.png">供应商管理</p>
						</c:when>
						<c:when test="${menu.name eq '采购商维护'}">
							<p class="nav_addclick"><img src="${ctxStatic}/images/nav_add.png">采购商管理</p>
						</c:when>
						<c:otherwise>
