				...
				<c:if test="${fns:getUser().admin}">
					...
					<div class="addiv fl">
						<sys:treeselect id="office" name="parent.id" value="${office.parent.id}" labelName="parentName" labelValue="${office.parent.name}"
							title="单位" url="/sys/office/treeData?platType=B&type=1" cssClass="required" notAllowSelectParent="false"/>
						<p class="redreset"></p>
					</div>
				</c:if>
				<c:if test="!${fns:getUser().admin}">
					<form:hidden path="parent.id" value = "${fns:getUser().company.id}"/>
				</c:if>
				<span>部门名称：</span>
				<div class="addiv fl">
