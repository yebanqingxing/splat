...
<head>
	...
	<script type="text/javascript">
		function addRole(){
			window.location.href="${ctx}/sys/role/supplierForm";
			return false;
		}
		function page(n,s){
			if(n) $("#pageNo").val(n);
			if(s) $("#pageSize").val(s);
			$("#searchForm").attr("action","${ctx}/sys/role/supplierList");
			$("#searchForm").submit();
	    	return false;
	    }
	</script>
...
<body>
	...
	<div class="addpolicy">
		<img src="${ctxStatic}/images/query.png">
		<p>操作员角色查询</p>
	</div>
	<div class="query clear">
		<form:form id="searchForm" modelAttribute="role" action="${ctx}/sys/role/supplierList" method="post" onclick="toSubmit()">
		  ...
		  <c:if test="${page.list == null||fn:length(page.list) ==0}">
		   <tr><td colspan="5" style="text-align: center;height: 40px;line-height: 40px;">暂无数据</td></tr>
			...
			<c:forEach items="${page.list}" var="role">
				<tr>
					<td><a href="${ctx}/sys/role/supplierForm?platType=S&id=${role.id}">${role.name}</a></td>
					<td><a href="${ctx}/sys/role/supplierForm?platType=S&id=${role.id}">${role.enname}</a></td>
					...
					<%-- <shiro:hasPermission name="sys:role:edit"> --%>
						<td>
							 ...
							 <%-- <c:if test="${(role.sysData eq fns:getDictValue('是', 'yes_no', '1') && fns:getUser().admin)||!(role.sysData eq fns:getDictValue('是', 'yes_no', '1'))}">
							  ...
							  <a href="${ctx}/sys/role/supplierForm?platType=S&id=${role.id}" class="btn btn-xs red">修改 <i class="fa fa-edit"></i></a>
		    				 <a href="${ctx}/sys/role/delete?platType=S&id=${role.id}" onclick="return confirmx('确认要删除该角色吗？', this.href)" class="btn btn-xs purple">删除 <i class="fa fa-times"></i></a>
						</td>
					<%-- </shiro:hasPermission>	 --%>
				</tr>
			</c:forEach>
