...
<body>
	...
	<form:form id="searchForm" modelAttribute="testTree" action="${ctx}/test/testTree/" method="post" class="breadcrumb form-search">
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-sm green" type="submit" style="margin-top:5px;" value="查询"/></li>
	...
	<script type="text/template" id="treeTableTpl">
		...
		<tr id="{{row.id}}" pId="{{pid}}">
			...
			<shiro:hasPermission name="test:testTree:edit"><td>
   				<a href="${ctx}/test/testTree/form?id={{row.id}}" class="btn btn-xs red">修改 <i class="fa fa-edit"></i></a>
				<a href="${ctx}/test/testTree/delete?id={{row.id}}" onclick="return confirmx('确认要删除该树结构及所有子树结构吗？', this.href)" class="btn btn-xs purple">删除 <i class="fa fa-times"></i></a>
                <a href="${ctx}/test/testTree/form?parent.id={{row.id}}" class="btn btn-xs green">添加下级树结构 <i class="fa fa-plus"></i></a>
