...
<body>
	...
	<form:form id="searchForm" modelAttribute="testData" action="${ctx}/test/testData/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" style="margin-top:5px;" class="btn btn-sm green" type="submit" value="查询"/></li>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed flip-content">
		...
		<c:if test="${page.list == null||fn:length(page.list) ==0}">
		      <tr><td colspan="8" style="text-align: center;height: 40px;line-height: 40px;">暂无数据</td></tr>
		</c:if>
		<c:forEach items="${page.list}" var="testData">
			<tr>
				<td>
				    <a href="${ctx}/test/testData/form?id=${testData.id}">${testData.user.name}</a>
				</td>
				<td>
				...
				<shiro:hasPermission name="test:testData:edit"><td>
				    <a href="${ctx}/test/testData/form?id=${testData.id}" class="btn btn-xs red">修改 <i class="fa fa-edit"></i></a>
    				<a href="${ctx}/test/testData/delete?id=${testData.id}" onclick="return confirmx('确认要删除该单表吗？', this.href)" class="btn btn-xs purple">删除 <i class="fa fa-times"></i></a>
