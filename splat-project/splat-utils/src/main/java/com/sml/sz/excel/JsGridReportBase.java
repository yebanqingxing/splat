...
public class JsGridReportBase {
	 ...
	 * @throws Exception
	 */
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData,String image) throws Exception {
		...
		if (image != null && sheet != null) {
			...
			int poiy = 38;
				...
				try {
					System.out.println("读取分析图……");
					bufferImg = ImageIO.read(new File(image));
					
					System.out.println("写入分析图……");
					ImageIO.write(bufferImg, "png", byteArrayOut);
					System.out.println("设置分析图……");
					// anchor主要用于设置图片的属性
					anchor = new HSSFClientAnchor(0, 0, 100, 50, (short) 5,
							poix, (short) 12, poiy);
					anchor.setAnchorType(3);
					poix += 20;
					poiy += 20;
					// 插入图片
					System.out.println("插入分析图……");
	 ...
	 * @throws Exception
	 */
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, HashMap<String, HSSFCellStyle> styles, String creator, List<TableData> tableDataLst,String[] images) throws Exception {
		...
		for(TableData tableData : tableDataLst){
			...
			TableHeaderMetaData headerMetaData = tableData.getTableHeader();// 获得HTML的表头元素
			 sheet = wb.createSheet(sheetTitle);// 在Excel工作簿中建一工作表
			sheet.setDisplayGridlines(false);// 设置表标题是否有表格边框
			wb.cloneSheet(0);
		...
		if (images != null && sheet != null) {
			...
			for (int i = 1; i <= images.length; i++) {
				byteArrayOut = new ByteArrayOutputStream();
				try {
					bufferImg = ImageIO.read(new File(images[i - 1]));
					ImageIO.write(bufferImg, "png", byteArrayOut);
					// anchor主要用于设置图片的属性
					anchor = new HSSFClientAnchor(0, 0, 100, 50, (short) 2,
							poix, (short) 7, poiy);
					anchor.setAnchorType(3);
					poix += 20;
					poiy += 20;
	 ...
	 * @throws Exception
	 */
	public HSSFWorkbook writeSheet(HSSFWorkbook wb, String title, HashMap<String, HSSFCellStyle> styles, String creator, TableData tableData,String[] images) throws Exception {
	 ...
	 *         bgcolor:#CAEAFE; </style>
	 */
	public void exportToExcel(String title, String creator, TableData tableData,String[] images)
			throws Exception {
		...
		wb = writeSheet(wb,title,styles,creator,tableData,images);//写入工作表
	...
	public void exportToExcel(String title, String creator, List<TableData> tableDataLst,String[] images)
	throws Exception {
	HSSFWorkbook wb = new HSSFWorkbook();// 创建新的Excel 工作簿
	HashMap<String, HSSFCellStyle> styles = initStyles(wb);// 初始化表头样式
	...
	for(TableData tableData : tableDataLst){
		String sheetTitle = tableData.getSheetTitle();
		sheetTitle = sheetTitle==null||sheetTitle.equals("")?"sheet"+i:sheetTitle;
		if(images!=null&&images[i-1]!=null&&StringUtils.isNotBlank(images[i-1])){
			//System.out.println(tableData.getSheetTitle());
			wb = writeSheet(wb,tableData.getSheetTitle(),styles,creator,tableData,images);//写入工作表
		}else {
			wb = writeSheet(wb,tableData.getSheetTitle(),styles,creator,tableData);//写入工作表
		}
