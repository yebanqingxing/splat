	...
	<div class="aircrafble">
		<table border="0" class="airctable" id="singlePrice">
			...
			<tr>
				...
				<td>&nbsp;<fmt:formatNumber pattern="0.0" value="${pls.supplierVoidCharge }"></fmt:formatNumber> </td>
				<%-- <td>&nbsp;<fmt:formatNumber pattern="0.0" value="${pls.supplierOtherCharge }"></fmt:formatNumber> </td>
				<td>&nbsp;${pls.supplierVoidCharge + pls.supplierOtherCharge}</td> --%>
			</tr>
			</c:forEach>
		</table>
	</div>
...
<div class="aircraft">
	<div class="single">
		<div class="singlecon">
			<ul class="clear">
				<li class="redreset"><span>&lowast;</span>废票结算总价：${detail.supplierTotSettlementPrice }</li>
			</ul>
		</div>
	</div>
</div>
...
<form id="form01" action="${ctx }/order/voidsheetDetail/confirm" method="post">
<!-- 留言 -->
...
<div class="aircraft">
	...
	<div class="rosizehead">
        ...
        <button type="button" onclick="confirm(7)">督促供应商核实废票</button>
        </c:if>
		<c:if test="${detail.currentStatus  == 4 && detail.voidsheetStatus != 2}">
			<button type="button" onclick="confirm(6)">确认废票</button>
			<button type="button" onclick="confirmOrder(2)">取消废票</button>
		</c:if>
		<c:if test="${detail.currentStatus  == 1 && detail.voidsheetStatus != 2}">
		<button type="button" onclick="confirmOrder(2)">取消废票</button>
		</c:if>
...
<!-- 取消订单隐藏域 -->
<input  type="hidden" id="hid_voidsheetStatus" name="hid_voidsheetStatus"  />
<!-- 订单号 -->
...
$(document).ready(function(){
	$("#singlePrice input[type !='hidden']").change(function(){
		var arr=$(this).parent().parent().find("input");
		var total=parseFloat($(arr[4]).val())+parseFloat($(arr[5]).val())
		$(arr[6]).val(Number(total).toFixed(2));
		...
		$("#voidorderDetail_totalPrice").val(Number(total).toFixed(2));
	});	
