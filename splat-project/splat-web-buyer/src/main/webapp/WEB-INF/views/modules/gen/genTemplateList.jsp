...
<body>
	...
	<div id="messageBoxError" class="alert alert-error"><button data-dismiss="alert" class="close">×</button>
		代码模板管理，已废弃！模板管理改为XML配置方式，见  /src/main/java/com/sml/sz/modules/gen/template 文件夹
	</div>
	<sys:message content="${message}"/>
	<table id="contentTable" class="table table-striped table-bordered table-condensed flip-content">
		<thead class="flip-content"><tr><th>名称</th><th>分类</th><th>备注</th><shiro:hasPermission name="gen:genTemplate:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
		...
		<c:forEach items="${page.list}" var="genTemplate">
			<tr>
				...
				<shiro:hasPermission name="gen:genTemplate:edit"><td>
    				<a href="${ctx}/gen/genTemplate/form?id=${genTemplate.id}" class="btn btn-xs red">修改  <i class="fa fa-edit"></i></a>
					<a href="${ctx}/gen/genTemplate/delete?id=${genTemplate.id}" onclick="return confirmx('确认要删除该代码模板吗？', this.href)" class="btn btn-xs purple">删除  <i class="fa fa-times"></i></a>
