...
<body>
	...
	<div class="query">
		<form:form id="searchForm" modelAttribute="role" action="${ctx}/sys/role/list" method="post" submit="return tosubmit()">
			<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
			<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			<span class="query-span" style="width: 90px; display: inline-block;">角色名称：</span>
			<form:input path = "name" htmlEscape="false" maxlength="50" class="querycode"/>
			<span class="query-span">状态信息：</span>
			<form:select path="useable"  class="selectquery" style="margin-right: 20px;" >
				<form:option value = "1" label = "启用"/>
				<form:option value = "0" label = "禁用"/>
			</form:select>
			<button class="addquery" onclick = "submit()">查询</button>
			<a href = "${ctx}/sys/role/form"><button class="addquery" onclick = "notSubmit()">新增</input></a>
		</form:form>
	</div>
