//download by http://www.codefans.net
	(function($) {
		...
		$.suggest = function(input, options) {
			...
			$input.blur(function() {
				if($.trim($(this).val())==''){
					$(this).val('中文/拼音');//显示热门城市列表
					$(this).siblings(".arrcity_3word").val("");
				}
			...
			$input.click(function(){
				...
				var q = text.length;
				var fastindex = text.lastIndexOf('/') + 1;
				var qtext = text.substring(fastindex,q);
			//	alert(qtext);
				displayItems(qtext);
//				$(this).select();  鼠标获取焦点时全选文字
			});
			...
			function processKey(e) {
				...
				if ((/27$|38$|40$/.test(e.keyCode) && $results.is(':visible')) ||
					(/^13$|^9$|^191$|^111$/.test(e.keyCode) && getCurrentResult())) {
			...
			function displayItems(items) {
				var html = '';
				if (items=='') {//热门城市遍历
					//options.hot_list=eval("("+options.hot_list+")") ;
					//alert(options.hot_list[0].length);
			...
			function selectCurrentResult() {
				...
				if ($currentResult) {
					var inlength = $input.val().lastIndexOf('/') + 1;
					var inval = $input.val().substring(0,inlength);
					if(inval == '中文/'){
						inval = '';
					}
					
					var airport = $currentResult.children('a').html().replace(/<span>.+?<\/span>/i,'');
					if(airport.substring(airport.length-2,airport.length) == '机场'){
						$input.val(inval.replace(/[^\u4e00-\u9fa5\/\a-zA-Z\0-9]/g,"") + $currentResult.attr("rel") + '/');
					}else{
						$input.val(inval.replace(/[^\u4e00-\u9fa5\/\a-zA-Z\0-9]/g,"") + airport + '/');
					}
					if($input.siblings(".arrcity_3word").val() == ''){
						$input.siblings(".arrcity_3word").val($currentResult.attr("rel"));
					}else{
						var inputval = $input.val();
						var numval = inputval.split("/");
						var relvalok = '';
						for (var i = 0; i < options.source.length; i++) {//国内城市匹配
							for(var k = 0;k<100;k++){
								if(options.source[i] == undefined){
							...
							for(var j=0;j<numval.length;j++){
								if(numval[j] == options.source[i][1] || numval[j] == options.source[i][0]){
									relvalok += options.source[i][0] +'/';
								}
							}
						}
						$input.siblings(".arrcity_3word").val(relvalok);
					}
					$results.hide();
