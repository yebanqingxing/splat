...
<body>
	...
	<form:form id="searchForm" modelAttribute="testDataMain" action="${ctx}/test/testDataMain/" method="post" class="breadcrumb form-search">
		...
		<ul class="ul-form">
			...
			<li class="btns"><input id="btnSubmit" class="btn btn-sm green" type="submit" style="margin-top:8px;" value="查询"/></li>
		...
		<c:forEach items="${page.list}" var="testDataMain">
			<tr>
				...
				<shiro:hasPermission name="test:testDataMain:edit"><td>
				    <a href="${ctx}/test/testDataMain/form?id=${testDataMain.id}" class="btn btn-xs red">修改 <i class="fa fa-edit"></i></a>
    				<a href="${ctx}/test/testDataMain/delete?id=${testDataMain.id}" onclick="return confirmx('确认要删除该主子表吗？', this.href)" class="btn btn-xs purple">删除 <i class="fa fa-times"></i></a>
