		...
		<div class="ordercon">
			<div class="orderconhead">
				<ul class="orderconul clear">
					...
					<c:if test="${detail.voidsheetStatus == '1' }">已提交</c:if>
			    	<c:if test="${detail.voidsheetStatus == '2' }">已拒单</c:if>
			    	<c:if test="${detail.voidsheetStatus == '3' }">已核单</c:if>
			    	<c:if test="${detail.voidsheetStatus == '4' }">已废票</c:if>
			    	<c:if test="${detail.voidsheetStatus == '5' }">已删除</c:if>
					</span></li>
					<li class="ordermargin">|</li>
...
<div class="aircraft">
	...
	<div class="aircrafble">
		...
		<c:forEach items="${passengerList }" var="voidsheetPassenger">
	<tr>
		<td>
		...
		<td>
		<c:if test="${voidsheetPassenger.certType == '0' }">身份证</c:if> 
		...
		<c:if test="${voidsheetPassenger.certType == '4' }">台胞证</c:if> 
		<c:if test="${voidsheetPassenger.certType == '5' }">回乡证</c:if> 
		<c:if test="${voidsheetPassenger.certType == '8' }">其他</c:if>
		<c:if test="${voidsheetPassenger.certType == '9' }">unknown</c:if> 
		</td>
...
<div class="aircraft">
	...
	<div class="rosizehead">
		<textarea></textarea>
		<div class="butbotom">
			<button ><a href="<%=request.getContextPath()%>/a/order/supplier/voidsheetDetail/editsupplierVoidsheetDetail?voidsheetStatus=3&voidsheetNo=${detail.voidsheetNo }">确认废票</a></button>
			<button ><a href="<%=request.getContextPath()%>/a/order/supplier/voidsheetDetail/editsupplierVoidsheetDetail?voidsheetStatus=2&voidsheetNo=${detail.voidsheetNo }">拒绝废票</a></button>
		</div>	
	</div>
</div>
</body>
