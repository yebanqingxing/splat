...
<body>
	...
	<c:choose>
		<c:when test="${empty genTable.name}">
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/form" method="post" class="form-horizontal" style="height:300px;">
				<form:hidden path="id"/>
				...
				<div class="control-group">
					...
					<div class="form-actions">
						<input id="btnSubmit" class="btn btn-sm btn-primary" type="submit" value="下一步"/>&nbsp;
						<input id="btnCancel" class="btn btn-sm default" type="button" value="返 回" onclick="history.go(-1)"/>
					</div>
				</div>
			</form:form>
		</c:when>
		<c:otherwise>
			<form:form id="inputForm" modelAttribute="genTable" action="${ctx}/gen/genTable/save" method="post" class="form-horizontal">
				...
				<fieldset>
					...
					<div class="control-group">
						<table id="contentTable"  class="table table-bordered table-striped table-condensed flip-content">
							<thead class="flip-content"><tr><th title="数据库字段名">列名</th><th title="默认读取数据库字段备注">说明</th><th title="数据库中设置的字段类型及长度">物理类型</th><th title="实体对象的属性字段类型">Java类型</th>
							...
							<tbody>
							<c:if test="${genTable.columnList == null||fn:length(genTable.columnList) ==0}">
							      <tr><td colspan="15" style="text-align: center;height: 40px;line-height: 40px;">暂无数据</td></tr>
							 </c:if>
							<c:forEach items="${genTable.columnList}" var="column" varStatus="vs">
								<tr${column.delFlag eq '1'?' class="error" title="已删除的列，保存之后消失！"':''}>
									...
									<td>
										<select name="columnList[${vs.index}].javaType" class="required" style="width:100px;margin:0px;">
									...
									<td>
										<select name="columnList[${vs.index}].queryType" class="required" style="width:90px;margin-top:0px;">
									...
									<td>
										<select name="columnList[${vs.index}].showType" class="required" style="width:100px;margin-top:0px;">
				...
				<div class="form-actions">
					<shiro:hasPermission name="gen:genTable:edit"><input id="btnSubmit" class="btn green" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
