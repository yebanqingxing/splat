...
<body>
	...
	<form:form id="inputForm" modelAttribute="user" action="${ctx}/sys/user/save" method="post" class="form-horizontal">
		...
		<form:hidden id = "office_id" path="company.id" value = ""/>
		<form:hidden path="platType" value = "S"/>
		
		<div class="effect-head clear" style="margin-top:20px;margin-bottom:30px;">
			<p>操作员<shiro:hasPermission name="sys:user:edit">${not empty user.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="sys:user:edit">查看</shiro:lacksPermission>：</p>
		...
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="company" name="office.id" value="${not empty user.id?user.company.id:''}"  labelName="officeName" labelValue="${not empty user.id?user.company.name:''}" notAllowSelectRoot="false"
					title="所属部门" url="/sys/office/treeData?platType=S" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
                <sys:treeselect id="role" name="roleIdList" value="${not empty user.id?user.roleIds:''}"  labelName="role.name" labelValue="${not empty user.id?user.roleNames:''}"
					title="角色" url="/sys/role/treeDate" cssClass="required"/><span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<c:if test="${empty user.id }">
			...
			<div class="control-group">
				...
				<div class="controls">
					<input id="confirmNewPassword" name="password" type="password" value="" maxlength="50" minlength="3" equalTo="#newPassword" class="required"/>
					<span class="help-inline"><font color="red">*</font> </span>
				</div>
			</div>
		</c:if>
		...
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="phone" class="required" htmlEscape="false" maxlength="100"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
				<form:input path="mobile" class="required" htmlEscape="false" maxlength="100"/>
				<span class="help-inline"><font color="red">*</font> </span>
			</div>
		</div>
		<div class="control-group">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type = "text" />
				<input type = "button" value = "修改" class="	btn btn-sm green"/>
			</div>
		</div>
		<div class="form-actions">
			...
			<%-- </shiro:hasPermission> --%>
			<input id="btnCancel" class="btn default" type="button" value="返 回" onclick="window.location.href = '${ctx}/sys/user/supplierList'"/>
		</div>
	</form:form>
</body>
</html>