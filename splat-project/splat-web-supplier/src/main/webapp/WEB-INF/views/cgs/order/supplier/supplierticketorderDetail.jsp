...
<form action="${ctx }/order/supplier/tbTicketorderDetail/editTicketorderDetailStatus?orderNo=${detail.orderNo }" method="post">
...
<!-- 					<li class="liwid"><span>操作历史&or;</span></li> -->
				...
				<ul class="clear">
					...
					<li>订单总费用：<input value="${detail.totalPrice }"  readonly="readonly" id="ticketorderDetail_totalPrice" name="totalPrice" style="width: 60px" /></li>
					<li>机票采购费用：<input value="${detail.settlementPrice }" name="settlementPrice" id="ticketorderDetail_settlementPrice" readonly="readonly" style="width: 60px" /></li>
					<li>机票税费：<input value="${detail.dutyPrice }" readonly="readonly"  id="ticketorderDetail_dutyPrice" name="dutyPrice" style="width: 60px" /></li>
					<li>报销凭证费：<input value="${detail.pzPrice }" name="pzPrice" id="ticketorderDetail_pzPrice" readonly="readonly" style="width: 60px" /></li>
...
<div class="aircraft">
	...
	<div class="aircrafble">
		<table border="0" class="airctable" id="sing_detail">
			...
			<c:forEach items="${singlePriceList }" var="spl" varStatus="idx">
				<tr>
				<input type="hidden" value="${spl.id }" name="ticketorderSinglePrices[${idx.index }].id"/>
				<input type="hidden" value="${spl.orderNo }" name="ticketorderSinglePrices[${idx.index }].orderNo"/>
					<td><input type="hidden" value="${spl.passengerName }" name="ticketorderSinglePrices[${idx.index }].passengerName" style="width: 65px" />${spl.passengerName }</td>
					<td>
					<c:if test="${spl.passengerType == '0' }">成人</c:if>
					<c:if test="${spl.passengerType == '1' }">儿童</c:if>
					<c:if test="${spl.passengerType == '4' }">婴儿</c:if>
					<input type="hidden" value="${spl.passengerType }" name="ticketorderSinglePrices[${idx.index }].passengerType" style="width: 65px" /></td>
					<td>
						<input value="${spl.distributorTicketPrice }" tip="_pm" name="ticketorderSinglePrices[${idx.index }].distributorTicketPrice" style="width: 65px" />
					</td>
					<td><input value="${spl.supplierTicketPrice }"  name="ticketorderSinglePrices[${idx.index }].supplierTicketPrice" style="width: 65px"/></td>
					<td><input value="${spl.distributorCommission }" name="ticketorderSinglePrices[${idx.index }].distributorCommission" style="width: 45px"/></td>
					<td><input value="${spl.supplierCommission }" name="ticketorderSinglePrices[${idx.index }].supplierCommission" style="width: 55px"/></td>
					<td><input value="${spl.distributorSettlementPrice }"  name="ticketorderSinglePrices[${idx.index }].distributorSettlementPrice" style="width: 55px"/></td>
					<td><input value="${spl.taxCn }" tip="_tx" name="ticketorderSinglePrices[${idx.index }].taxCn" style="width: 55px"/></td>
					<td><input value="${spl.supplierSettlementPrice }" name="ticketorderSinglePrices[${idx.index }].supplierSettlementPrice" tip="_t" style="width: 65px"/></td>
				</tr>
			</c:forEach>
	...
	<div class="singlecon">
			<ul class="clear">
				<li><span class="redreset">&lowast;</span>订单总费用：<input value="${detail.totalPrice }"  readonly="readonly" id="ticketorderDetail_totalPrice_1"  style="width: 60px" /></li>
				<li>机票：<input value="${detail.settlementPrice }" id="ticketorderDetail_settlementPrice_1" readonly="readonly" style="width: 60px" /></li>
				<li>机票税费：<input value="${detail.dutyPrice }" readonly="readonly"  id="ticketorderDetail_dutyPrice_1"  style="width: 60px" /></li>
				<li>报销凭证费：<input value="${detail.pzPrice }"  id="ticketorderDetail_pzPrice_1" readonly="readonly" style="width: 60px" /></li>
<!-- 				<li>保险费：<input value="0" readonly="readonly" style="width: 60px" /></li> -->
...
<div class="aircraft">
	...
	<textarea name="tk_message" cols="50" rows="5" id="textarea" onKeyDown="textdown(event)"
		...
		<input type="hidden" name="currentStatus" id="hid_currentStatus" />
			<div class="butbotom">
			<c:if test="${ (detail.currentStatus != 5 && detail.currentStatus != 1 && detail.payStatus == 1 && detail.orderStatus != 7 )}">
				<button class="addquery" type="submit" onclick="order_operation('3');">督促采购商支付</button>
			</c:if>
			<c:if test="${(detail.orderStatus  == 1 && detail.currentStatus == 1  )  }">
				<button class="addquery addche" type="submit" onclick="order_operation('2')">核实订单</button>
				<button class="addquery addche" type="submit" onclick="order_operation('5')">拒绝订单</button>
			</c:if>
			<c:if test="${(detail.orderStatus  == 1 && detail.currentStatus == 9 && detail.payStatus == 3 )  }">
				<button class="addquery addche" type="submit" onclick="order_operation('10')">确认全额退款</button>
               <%--  <a  href="${ctx }/order/supplier/tbTicketorderDetail/refundsheetDetailAAAAAA?orderNo=${detail.orderNo}&currentStatus=12"><button class="addquery "  type="button">拒绝全额退款</button></a> --%>
            </c:if>
