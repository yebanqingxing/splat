...
<form id="form01" action="${ctx }/order/supplier/voidsheetDetail/editsupplierVoidsheetDetail" method="post" enctype="multipart/form-data">
<!-- 废费金额 -->
<div class="colorhei"></div>
<div class="aircraft">
	<div class="effect-head">
	...
	<div class="aircrafble">
		<table border="0" class="airctable" id="singlePrice">
			...
			<tr>
				<input type="hidden" value="${pls.id }" name="voidsheetPassengers[${idx.index }].id"/>
				<input type="hidden" value="${pls.voidsheetNo }" name="voidsheetPassengers[${idx.index }].voidsheetNo"/>
				<input type="hidden" value="${pls.passengerName }" name="voidsheetPassengers[${idx.index }].passengerName"/>
				<%-- <td><input type="text"  value="${pls.passengerName }" style="width: 160px" disabled="disabled" /></td> --%>
				<td>&nbsp;${pls.passengerName }</td>
				
				<td><input type="text" class="supplierVoidCharge" name="voidsheetPassengers[${idx.index }].supplierVoidCharge" tip="_sell" style="size: 160px;" value="<fmt:formatNumber value='${pls.supplierVoidCharge }' pattern="0.0"></fmt:formatNumber>"/></td>
				<%-- <td><input type="text" name="voidsheetPassengers[${idx.index }].supplierOtherCharge"  tip="_tax" style="size: 160px;" value="${pls.supplierOtherCharge }"/></td>
				<td><input type="text" name="voidsheetPassengers[${idx.index }].supplierSmtPrice" tip="_total" style="size: 160px;" value="${pls.supplierSmtPrice }"></td> --%>
			</tr>
			</c:forEach>
		</table>
	</div>
	...
	<div class="single">
		<div class="singlecon">
			<ul class="clear">
				...
				<li class="redreset"><span>&lowast;</span>废票结算总价：<input readonly="readonly" type="text" value="<fmt:formatNumber value='${detail.supplierTotSettlementPrice }' pattern='0.0'></fmt:formatNumber>" name="supplier_tot_settlement_price"  id="voidorderDetail_totalPrice"/></li>
			</ul>
		</div>
	</div>
</div>
...
<div class="aircraft">
	...
	<div class="rosizehead">
		...
		<div class="butbotom">
			<c:if test="${detail.voidsheetStatus == 1 && detail.currentStatus == 1 }">
			<button type="button" onclick="order_operation2(3)">审核拒绝</button>
			<button type="button" onclick="order_operation(4)">审核通过</button>
			</c:if>
			<c:if test="${detail.currentStatus == 6  && detail.voidsheetStatus != 2 && detail.voidsheetStatus != 3}">
				<button type="button" onclick="order_operation1(5)">确认废票</button>
			</c:if>
			<button class="addquery addche" type="button" onclick="window.history.back(-1);">返回</button>
		</div>	
	</div>
</div>
<input type="hidden" id="hid_currentStatus" name="hid_currentStatus">
<!-- 订单号 -->
<input type="hidden" name="voidsheetNo" value="${detail.voidsheetNo }"/>
</form>
<script type="text/javascript">
function order_operation2(type){
	$("#hid_currentStatus").val(type);
	$("#form01").submit();
}
function order_operation1(type){
	$("#hid_currentStatus").val(type);
	$("#form01").submit();
}
function order_operation(type){
	$("#hid_currentStatus").val(type);
	if($("#voidorderDetail_totalPrice").val() != '' && $("#voidorderDetail_totalPrice").val()*1 > 0 ){
		$("#form01").submit();
	}else{
		alert("请完善金额信息");
	}
...
$(document).ready(function(){
	$(".supplierVoidCharge").change(function (){
		var partn=/^[+-]?\d+(\.\d+)?$/;
		if(!partn.test($(this).val())){
			$(this).val("0.0");
			 alert("请正确输入金额");
			 return false;
		}
		var total = 0;
		$("#singlePrice input[type !='hidden']").each(function(){
			if($(this).attr("tip") == '_sell'){
				total += parseFloat($(this).val());
			}
			
		});
		$("#voidorderDetail_totalPrice").val(Number(total).toFixed(1));
