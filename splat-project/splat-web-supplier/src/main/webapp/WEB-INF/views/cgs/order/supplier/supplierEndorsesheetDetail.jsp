			...
			<div class="ordercon">
				<div class="orderconhead">
					<ul class="orderconul clear leftconul">
						...
						<li class="ordermargin">|</li>
						<li>操作状态：<span class="redreset"> <c:if
			 						test="${detail.currentStatus == 1 }">等待审核</c:if> 
									<c:if test="${detail.currentStatus == 2 }">审核通过</c:if>
									<c:if test="${detail.currentStatus == 3 }">审核拒绝</c:if>
									 <%-- <c:iftest="${detail.currentStatus == 4 }">等待审核</c:if> --%>
...
<!-- 						<li class="liwid"><span>操作历史&or;</span></li> -->
					...
					<ul class="clear">
						<li class="colorli">改签费用总金额：<input type="text"
							style="width: 60px;" id="endorseTotalCost1" disabled="disabled" value="${detail.endorseTotalCost }"/></li>
					</ul>
				</div>
			</div>
		</div>
					...
					<c:forEach items="${tSegmentList }" var="segment">
						<div class="marin">
							<ul class="clear ultwo">
								...
								<li class="licenter">
									<p>
										&nbsp;<fmt:formatDate value="${segment.departureTime}" pattern="yyyy-MM-ss HH:mm:ss" />
									</p>
									<p>&nbsp;${segment.departureAddress }</p>
								</li>
<!-- 								<li class="licenter"> -->
<%-- 									<p>&nbsp;${segment.duration}</p> <img src="${ctxStatic}/order/images/pnrhang.png"> <!-- 							<p class="transfer">1程中转</p> --> --%>
<!-- 								</li> -->
								<li class="licenter">
									<p>&nbsp;<fmt:formatDate value="${segment.arriveTime}" pattern="yyyy-MM-ss HH:mm:ss" />
									</p>
			...
			<table border="0" class="airctable">
				...
				<c:forEach items="${singlePriceList }" var="singlePrice">
					<tr>
						...
						<td>&nbsp; 
						<c:if test="${singlePrice.passengerType == '0' }">成人</c:if> <c:if
								test="${singlePrice.passengerType == '1' }">儿童</c:if> <c:if
								test="${singlePrice.passengerType == '4' }">婴儿</c:if></td>
	...
	<form id="form01"
		action="${ctx }/order/supplier/endorsesheetDetail/editEndorsesheetDetail"
		method="post">
		<!-- 改签费用金额 -->
		<div class="colorhei"></div>
		<div class="aircraft">
				...
				<table border="0" class="airctable" id="table01">
					...
					<c:forEach items="${passengerList }" var="passegner"
						...
						<tr>
							...
							<td>&nbsp; <input type="text" disabled="disabled" style="width: 60px"
								tip="_total"
								name="endorsesheetPassengers[${passIndex.index}].endorseSurcharge"
								value="${passegner.endorseSurcharge}" />
							</td>
		...
		$("#table01 input[type !='hidden']").change(function() {
			...
			$(arr[7]).val(Number(total).toFixed(1));
			//获取每一行的值进行加
...
<div class="aircraft">
    ...
    <div class="aircrafble showhide" style="height:144px; overflow-y:scroll;">
		...
		<div class="butbotom">
			<c:if test="${(detail.currentStatus == 1 || detail.currentStatus == 4)  && detail.endorseStatus == 1}">
	...
	<script type="text/javascript">
		function submitOrder(type) {
			$("#hid_currentStatus").val(type);
				...
				if ($("#endorseTotalCost").val() == 0 || '' == $("#endorseTotalCost").val()){
