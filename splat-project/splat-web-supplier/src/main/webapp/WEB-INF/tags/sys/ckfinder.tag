...
<ol id="${input}Preview"></ol>
  <c:if test="${!readonly}">
    <a href="javascript:" onclick="${input}FinderOpen();" class="btn btn-sm green">${selectMultiple?'添加':'选择'} <i class="fa fa-link"></i></a>&nbsp;<a href="javascript:" onclick="${input}DelAll();" class="btn btn-sm grey-cascade">清除 <i class="fa fa-times"></i></a>
 </c:if>
	...
	function ${input}Preview(){
		...
		for (var i=0; i<urls.length; i++){
			if (urls[i]!=""){//<c:if test="${type eq 'thumb' || type eq 'images'}">
				...
				li += "&nbsp;&nbsp;<c:if test="${!readonly}"><a href=\"javascript:\" onclick=\"${input}Del(this);\">&times</a></c:if></li>";
