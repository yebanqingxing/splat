...
public class PNRXML {
	...
	private static Logger logger = LoggerFactory.getLogger(PNRXML.class);
	...
	List qtes;
	List fcs;
	List baseprice=new ArrayList();

	List exchages=new ArrayList();
	...
	public List getExchages() {
		return exchages;
	}



	public void setExchages(List exchages) {
		this.exchages = exchages;
	}



	public List getBaseprice() {
		return baseprice;
	}



	public void setBaseprice(List baseprice) {
				...
				if(rowData.indexOf(".")>0 && rowData.substring(rowData.indexOf(".")+1).indexOf(" ")==0){
					...
					if(rowData!=null){
						if(!"".equals(rowData.trim())){
							...
							if(pnr1.length() > 0){
								if(rowData.length()>8&&rowData.substring(rowData.replaceAll("\r", "").length()-8).replaceAll(" ", "").equalsIgnoreCase(pnr1))
							...
							int startPassenger = source.indexOf("\n1.");
							if(startPassenger==-1)
								startPassenger= source.indexOf("1.");
							source = source.substring(startPassenger);
							String[] pnrLineRows = source.split("\n");//按行拆分成一个数组
							int findLasrPassenger = findLasrPassenger(pnrLineRows);
							int lastPassLength=pnrLineRows[findLasrPassenger-1].trim().length();
							String pnrNUmber = pnrLineRows[findLasrPassenger-1].trim().substring(lastPassLength-6, lastPassLength);
							root.addElement("pnrCode").addAttribute("pnr", pnrNUmber);
				...
				if(rowData!=null){
					...
					if(rowData.indexOf(".")>0 && rowData.length()>2&&!rowData.substring(0, 3).contains("*")&&!rowData.startsWith("ENDOS")&&!rowData.startsWith(" ENDOS")){
						if(rowData.substring(rowData.indexOf(".")+1).indexOf(" ")==0){
							...
							//解析hui lv 
							if(rowData.trim().startsWith("RATE USED")){
								String[] split = rowData.replaceAll("\\s+", "").split("=");
								root.addElement("RateUse").addAttribute("rate", split[1].substring(0, split[1].length()-3));
							}
					...
					if(rowData.startsWith(attributeValue)||rowData.startsWith(" "+attributeValue)){
						...
						List<String> numberQ =null;
						if(exictsQ||exictsQSpace)
							numberQ = numberQ(fcLast);
						if(exictsQ){
						...
						if(stringFc.contains("X/")){
							...
							case 3:
								if((exictsQ||exictsQSpace)&&numberFcForIt==0) {// first:only have value of Q
									...
									createFcDocument(root, iTRate(fcLast),splitSpace[0],splitSpace[4],"IT",idenFc.get(t));
							...
							case 4 ://说明只含有2个Fc
								if((exictsQ||exictsQSpace)&&numberFcForIt==0) {
									...
									if(exictsQ){
										if(rankBool){
											for(int k=0;k<size-2;k++){
												if(k==0){//[SHA, KA, X/HKG, CX, BKK, M/IT, /CNX, KA, X/HKG, KA, SHA, M/IT, END, ROE6.514200, E]
													createFcDocument(root, getItRoe(fcLast),splitSpace[0],splitSpace[4],"IT",getNumByString(plusNumberQ),idenFc.get(t));

												}else if(k==1){
													createFcDocument(root, getItRoe(fcLast),splitSpace[6].substring(1, 4),splitSpace[0],"IT",idenFc.get(t));
												}
									...
									if(rankBool){
										for(int k=0;k<lists.size()-2;k++){
											...
											String firstToCity=splitSpace[4].substring(0, 3);
											if(firstToCity.contains("/")){
												firstToCity=splitSpace[6].substring(0, 3);
											}
											...
											if(k==0){
												...
												//[LHW, CZ, X/CAN, CZ, X/KUL, MH, KBR279.38/KUL, CZ, X/CAN, CZ, LHW215.68NUC495.06END, ROE6.514200XT]
											}else{
												if(splitSpace[5].length()>12){

													secondFromCity=splitSpace[5].substring(9, 12);
												}else if(splitSpace[4].length()>13){

													secondFromCity=splitSpace[4].substring(10, 13);
												}else if(splitSpace[6].length()>13){
													secondFromCity=splitSpace[6].substring(10, 13);
												}else if(splitSpace[5].length()>11){
													secondFromCity=splitSpace[5].substring(8, 11);
												}else if(splitSpace[5].contains("/")){
													secondFromCity=splitSpace[5].substring(8, 11);
												...
												}else if(!splitSpace[8].substring(0, 3).contains("/")){
													secondToCity=splitSpace[8].substring(0, 3);
												}else{
													secondToCity=splitSpace[0];
												}
												createFcDocument(root, (String)lists.get(3),secondFromCity,splitSpace[0],(String) lists.get(k),idenFc.get(t));
													...
													if(findFirstX){
														for(int k=0;k<lists.size()-2;k++){
															if(splitSpace.length==8){
																...
																//[BJS, CA<PA>HOU690.79UA, X/NYC, CA, BJS752.20NUC1442.99ENDROE6.514200XT]
																String firstTocity=splitSpace[2].substring(0, 3);
																if(firstTocity.contains("/")&&splitSpace[1].length()>9)
																	firstTocity=splitSpace[1].substring(6, 9);
																if(k==0){
																	createFcDocument(root, (String)lists.get(3),splitSpace[0],firstTocity,(String) lists.get(0),idenFc.get(t));

																}else{
																	String toCity=splitSpace[0];
																	if(splitSpace.length>5&&!toCity.equals(splitSpace[5].substring(0, 3)))
																		toCity=splitSpace[5].substring(0, 3);
																	if(toCity.contains("/"))
																		toCity=splitSpace[7].substring(0, 3);
																	createFcDocument(root, (String)lists.get(3),firstTocity,toCity,(String) lists.get(1),idenFc.get(t));
										...
										}else if(doublesAdd(lists.get(0),lists.get(1))!=objectToDouble(lists,2)){//2个fc  含有s值 ，停留值
											//[BJS, AF, X/PAR, AF, ALG330.04AF, PAR, AF, BJS330.04, 1S115.13NUC775.21END, ROE6.514200XT]
											for(int k=0;k<lists.size()-2;k++){
												if(k==0){
													createFcDocument(root, (String)lists.get(3),splitSpace[0],splitSpace[2].substring(2, 5),(String) lists.get(k),idenFc.get(t));

												}else{
													createFcDocument(root, (String)lists.get(3),splitSpace[2].substring(2, 5),splitSpace[0],(String) lists.get(k),idenFc.get(t));
							...
							case 5://说明只含有3个Fc
								if((exictsQ||exictsQSpace)&&numberFcForIt==0) {
									...
									if(exictsQ){
										...
										for (int j = 0; j < lists.size()-2; j++) {
											//[CAN, KQ, X/NBO, KQ, DKR624.78KQ, X/NBO, KQ, FIH, Q, DKRFIH15.00, 262.13KQ, X/NBO, KQ, CAN, Q, FIHCAN15.00, 617.88NUC1534.79END, ROE6.51420]

											if(j==0){

												createFcDocument(root, (String)lists.get(lists.size()-1),splitSpace[0],splitSpace[4].substring(0, 3),(String) lists.get(j),plusNumberQ,idenFc.get(t));

											}else if(j==1){
												createFcDocument(root, (String)lists.get(lists.size()-1),splitSpace[4].substring(0, 3),splitSpace[7],(String) lists.get(2),idenFc.get(t));

											}else if(j==2){
												createFcDocument(root, (String)lists.get(lists.size()-1),splitSpace[7],splitSpace[0],(String) lists.get(4),idenFc.get(t));
									...
									if(middleNumbers==2){
										//[SHA, CA, X/MIL, CA, LIS, M2486.87CA, X/FRA851.89CA, SHA2709.46NUC6048.22END, ROE6
										for(int k=0;k<lists.size()-2;k++){
											if(k==0){
												createFcDocument(root, (String)lists.get(4),splitSpace[0],splitSpace[4],(String) lists.get(k),idenFc.get(t));
											}else if (k==1){
												createFcDocument(root, (String)lists.get(4),splitSpace[4],splitSpace[6].substring(2, 5),(String) lists.get(k),idenFc.get(t));
											}else if (k==2){
												createFcDocument(root, (String)lists.get(4),splitSpace[6].substring(2, 5),splitSpace[0],(String) lists.get(k),idenFc.get(t));
											}

										}

										//[XMN, CZ, X/CAN483.55CZ, KTM, AI, X/DEL, M1333.24CZ, X/CAN, CZXMN231.80NUC2048.59END, ROE6.514200]
									}else if(middleNumbers==3){
							...
							case 4://2个fc
								if((exictsQ||exictsQSpace)&&numberFcForIt==0) {
									...
									if(exictsQ&&!exictsQSpace){
										...
										List<String> removeQlistFromLists = removeQlistFromLists(lists,numberQ);
										//[BJS, AC, YTO, 516.56AC, YVR, AC, BJS, Q, YTOBJS11.22, 605.60NUC1144.60END, ROE6.514200, X]
										for(int k=0;k<removeQlistFromLists.size()-2;k++){
											if(k==0){
												createFcDocument(root, (String)lists.get(lists.size()-1),splitSpace[0],splitSpace[2],(String) removeQlistFromLists.get(0),plusNumberQ,idenFc.get(t));

											}else if(k==1){
												String secondToCity=splitSpace[0];
												String secondFromCity=splitSpace[2];
												if(splitSpace[3].length()>10)
													secondFromCity=splitSpace[3].substring(8, 11);
												if(splitSpace[5].length()==3)
													secondToCity=splitSpace[5];
												createFcDocument(root, (String)lists.get(lists.size()-1),secondFromCity,secondToCity,(String) removeQlistFromLists.get(2),idenFc.get(t));
									...
									if(exictsQ){
										String plusNumberQ=plusNumberQ(numberQ);
										for(int k=0;k<size-2;k++){
											if(k==0){//[TSN, HX, HKG, M/IT, HX, TSN, M/IT, END, ROE6.514200EN]
												createFcDocument(root, getItRoe(fcLast),splitSpace[0],splitSpace[2],"IT",getNumByString(plusNumberQ),idenFc.get(t));

											}else if(k==1){

												createFcDocument(root, getItRoe(fcLast),splitSpace[2],splitSpace[0],"IT",idenFc.get(t));
									...
									if(lists.get(0).equals(lists.get(1))){
										if(rankBool){//[URC, CZ<FE>LED107.45/MOW, CZ, URC107.45NUC214.90END, ROE6.514200, X]
													 //[CAN, CZ, NYC353.07/LAX, CZ, CAN353.07NUC706.14END, ROE6.514200XT]
											for (int j = 0; j < lists.size()-2; j++) {
												if(j==0){
													String findToCity = findToCity(rowData, lists, j);
													createFcDocument(root, (String)lists.get(3),splitSpace[0],findToCity,(String) lists.get(0),idenFc.get(t));

												}else if(j==1){
													String secondToCity = findToCity(rowData, lists, j);
													List<String> findBeginRankCity = findBeginRankCity(rowData);
													createFcDocument(root, (String)lists.get(3),findBeginRankCity.get(0),splitSpace[0],(String) lists.get(1),idenFc.get(t));
										...
										//两个fc的情况  没有q
										if(doublesAdd(lists.get(0),lists.get(1))==objectToDouble(lists, 2)){
											for (int j = 0; j < lists.size()-2; j++) {
												if(j==0){
													createFcDocument(root, (String)lists.get(3),splitSpace[0],splitSpace[2].substring(0, 3),(String) lists.get(0),idenFc.get(t));
												}else if(j==1){
							...
							case 6://4个fc
								for (int j = 0; j < lists.size()-2; j++) {
									//[SHA, MU, HNL1596.51AS, SEA367.93AS, SFO129.30/LAX, MU, SHA1773.04NUC3866.78END, ROE6.514200, X]
									if(j==0){
										createFcDocument(root, (String)lists.get(5),splitSpace[0],splitSpace[2].substring(0, 3),(String) lists.get(0),idenFc.get(t));
									}else if(j==1){
										createFcDocument(root, (String)lists.get(5),splitSpace[2].substring(0, 3),splitSpace[3].substring(0, 3),(String) lists.get(1),idenFc.get(t));
									}else if(j==2){
										createFcDocument(root, (String)lists.get(5),splitSpace[3].substring(0, 3),splitSpace[4].substring(0, 3),(String) lists.get(2),idenFc.get(t));
									}else if(j==3){
										createFcDocument(root, (String)lists.get(5),splitSpace[4].substring(0, 3),splitSpace[0],(String) lists.get(3),idenFc.get(t));
									}
								}
								break;
							case 7://5个fc
					...
					if((rows[i].contains("QTE")||rows[i].contains("qte"))&&!rows[i].contains("*")){
						...
						for(int g=f;g<f+1;g++){
							...
							for (int m = i; m < (Integer) selectQteRows.get(g+1); m++) {
								rowData=rows[m];
								if((rowData.trim().startsWith(substringTotal)&&!rowData.contains("/"))||rowData.trim().startsWith("FARE")||rowData.trim().startsWith("TAX")){
									if((rowData.trim().startsWith(substringTotal)&&!rowData.contains("/"))){
								...
								if(rowData.trim().startsWith("FARE")){
									//Element rootElement = document.getRootElement();
									...
									//[, FARE, CNY, 1980, *IT, FARE*] 
									if(split1.length==6){
									if(rowData.contains("IT")){
										element.addAttribute("fare", split1[3])
										.addAttribute("fareUnit", split1[2])
										.addAttribute("type", "IT");
										;
									}else{
									...
									}else{//[FARE, CNY, 3420, *IT, FARE*]
										if(rowData.contains("IT")){
											element.addAttribute("fare", split1[2])
											.addAttribute("fareUnit", split1[1])
											.addAttribute("type", "IT");
											;
										}else{
											element.addAttribute("fare", split1[split1.length-1])
								...
								if(rowData.trim().trim().startsWith("TAX")){
									...
									Matcher matcher = pattern.matcher(rowData);
									String replaceAll = matcher.replaceAll(" ");
									String replaceFirst = replaceAll.replaceAll("[a-zA-Z]", "");//TAX   CNY      90CN CNY      14E7 CNY     130TS 
								...
								if(rowData.trim().startsWith("COMMISSION")&&rowData.trim().endsWith("PERCENT OF GROSS")){
									int beginCommisiion = rowData.indexOf("COMMISSION");
									int endCommisiion = rowData.indexOf("PERCENT OF GROSS");
									String commision = rowData.trim().substring(beginCommisiion+10, endCommisiion).trim();
									element.addAttribute("commission", commision);
								}
					...
					//解析基础票价
					String beginCity = element2.attributeValue("beginCity");
					String beginCityValue = getValueFromKey(beginCity);
					if(rowData.trim().length()==3&&(rowData.trim().startsWith(beginCityValue)||rowData.trim().startsWith(beginCity))){
						StringBuffer basePrice=new StringBuffer();
						for(int j=i+1;j<getFare(rows).get(0);j++){
							String[] split = rows[j].trim().replace("\\s+", " ").split(" ");
							basePrice.append(split[1]);
							if(j!=getFare(rows).get(0)-1)
								basePrice.append(",");
						}
						root.addElement("Base").addAttribute("basePrice", basePrice.toString());
	...
	public  void parsePassengerRow(Element root,String passenger,boolean includePNR){
		...
		for(int i=1;i<passengerInfo.length;i++){
			 ...
			 * 3.LUO/PING MR
			 * 4.ZHANG/HUILI MS
			 * [HOU/ZHIHUA, SD, HQ50CG]
			 * [LI/NUO]
			 */
			String[] split = name.split(" ");//
			String passIdentity=null;
			if(split.length!=1){
				passIdentity=split[split.length-1];//旅客身份
			}
			if(passIdentity!=null&&passIdentity.length()==6)
				passIdentity=split[split.length-2];
	...
	public boolean parsePNRXML(Document document) {
		...
		//获取基础票价
		for(Iterator  i= outRoot.elementIterator("Base");i.hasNext();){
			Element foo = (Element) i.next();
			form = new PNRContent();

			if(!trim(foo.attributeValue("basePrice")).equals("")){
				form.setBasePrice(foo.attributeValue("basePrice"));
			}
			baseprice.add(form);
		}

		//获取汇率
		for(Iterator  i= outRoot.elementIterator("RateUse");i.hasNext();){
			Element foo = (Element) i.next();
			form = new PNRContent();

			if(!trim(foo.attributeValue("rate")).equals("")){
				form.setExchangeRate(foo.attributeValue("rate"));
			}
			exchages.add(form);
		}
		...
		for (Iterator i = outRoot.elementIterator("qte"); i.hasNext();) {
			...
			if(!trim(foo.attributeValue("commission")).equals("")){
				form.setCommission(foo.attributeValue("commission"));
			}
			/*if(!trim(foo.attributeValue("fcNum")).equals("")){
				Fc fc=new Fc();
	...
	public static int stringNumbers(String str){ 
		String target = "X/";
		int count =0, start =0;
		while((start=str.indexOf(target,start))>=0){
			start += target.length();
			count ++;
		}
		return count;
	 ...
	 * @date 2016年3月30日
	 * 作用：判断有Q值  
			1---->20AUG16BJS AC X/YVR AC YTO Q BJSYTO11.22 875.01AC X/YVR AC B                    
			JS Q YTOBJS11.22 875.01NUC1772.46END ROE6.514200    " Q "

			2----> 04JUL16YTO AC BJS Q11.22 411.67//SHA AC YTO Q74.85Q BJSYTO11                    
			.22M493.26NUC1002.22END ROE1.336000      "Q "

			3----->09JUN16CGQ CA X/BJS CA MUC M245.61CA X/BJS CA CGQ303.18NUC54                    
					8.79END ROE6.514200 
			4----->20AUG16BJS CX X/HKG554.17CX X/YTO Q5.79CX YMQ M2226.17NUC278                    
6.					13END ROE6.514200
	 */
	public boolean exictsQSpace(String str){
		String replaceAll = str.replaceAll("CGQ", "CGG").replaceAll("AYQ", "AYY")
				.replaceAll("AMQ", "AMM").replaceAll("KMQ", "KMM")
				.replaceAll("YMQ", "YMM").replaceAll("YMQ", "YMM")
				.replaceAll("BAQ", "BAA").replaceAll("AAQ", "AAA")
				.replaceAll("ABQ", "ABB").replaceAll("PUQ", "PUU")
				.replaceAll("IQQ", "III").replaceAll("GDQ", "GDD")
				.replaceAll("ELQ", "ELL").replaceAll("CHQ", "CHH")
				.replaceAll("CPQ", "CPP").replaceAll("CBQ", "CBB")
				.replaceAll("BUQ", "BUU").replaceAll("BLQ", "BLL")
				.replaceAll("BIQ", "BII").replaceAll("ATQ", "ATT")
				.replaceAll("PNQ", "PNN").replaceAll("PWQ", "PWW")
				.replaceAll("PLQ", "PLL").replaceAll("MGQ", "MGG")
				.replaceAll("NTQ", "NTT").replaceAll("LPQ", "LPP")
				.replaceAll("MSQ", "MSS").replaceAll("BDQ", "BDD")
				.replaceAll("SVQ", "SVV").replaceAll("SRQ", "SRR")
				.replaceAll("SDQ", "SDD").replaceAll("SCQ", "SCC")
				.replaceAll("SFQ", "SFF").replaceAll("RMQ", "RMM")
				.replaceAll("NGQ", "NGG").replaceAll("THQ", "THH")
				.replaceAll("GOQ", "GOQ").replace("SQ", "SS")
				;
		Pattern p = Pattern.compile("Q ");
		boolean find = p.matcher(replaceAll).find();
		return find;
	}
	/**
	 * 
	...
	public String qteIdentity(String str){
		if(str.indexOf("/")==-1){
			...
			if(str.contains("QTE:")||str.contains("qte:")){
				int indexOf = str.indexOf(":");
				if(indexOf==-1){
					indexOf= str.trim().indexOf(" ");
				}
				int indexOf2 = str.indexOf("/");
				String identity = str.substring(indexOf+1,indexOf2);
				return identity;
			}else{
				int indexOf = str.indexOf("QTE");
				int indexOf2 = str.indexOf("/");
				String identity = str.substring(indexOf+3,indexOf2).trim();
				return identity;
			}
		}
	}
	 ...
	 * 	（0成人，1儿童，4婴儿）
	 */
	public static int findPasTypeByName(String name){
		if("CHD".equalsIgnoreCase(name)){
			return 1;
	 ...
	 */
	public static  int findPasIdentityByName(String name){
	...
	public List <String> getIdenFc(String[] rows ){
		List <String> lists =new ArrayList<String>();
		for (int i = 0; i < rows.length; i++) {
			String string = rows[i].trim().replaceAll("\\s+", " ");
			//if(rows[i+1].contains("qte")||rows[i+1].contains("QTE"))
			if(string.contains("qte:")||string.contains("QTE:")){
				int indexOf = string.indexOf("QTE");
				if(indexOf==-1)
					indexOf=string.indexOf("qte");
	...
	public List<Integer> getFare(String [] rows){
		List<Integer> lists=new ArrayList<Integer>();
		for (int i = 0; i < rows.length; i++) {
			String string = rows[i];

			if(string.trim().startsWith("FARE")){
				lists.add(i);
			}
	 ...
	 * 作用：相同城市不同的三字码
	 */
	public String getValueFromKey(String cityKey){
		Properties propertiesValue = PropertiesUtil.getPropertiesValue("pnr.properties");
		String cityValue =(String) propertiesValue.get(cityKey);
		if(cityValue==null){
			cityValue=cityKey;
		}
		return cityValue;
	...
	public  int findLasrPassenger(String rows[]){	
		for(int i=0;i<rows.length-1;i++){
			String substring = rows[i].substring(rows[i].indexOf(".")+1);
	 ...
	 * @auth 冯俊伟
	 * @date 2016年4月26日
	 * 作用：对于缺口层的话，28MAY16CAN CZ NYC353.07/-LAX CZ CAN353.07NUC706.14END ROE6.5 
	 * 取出缺口后的起飞城市
	 * 
	 */
	public static List<String> findBeginRankCity(String str){
		List<String> liStrings =new ArrayList<String>();
		String[] split = str.split("/-");
		for (int i = 1; i < split.length; i++) {
			liStrings.add(split[i].substring(0, 3));
		}
		return liStrings;
	 ...
	 * @auth 冯俊伟
	 * @date 2016年4月26日
	 * 作用：28MAY16CAN CZ NYC353.07/-LAX CZ CAN353.07NUC706.14END ROE6.5  
	 * 根据353.07取出NYC 
	 */
	public String findToCity(String str,List<String> rowdata,int i){
		String[] split = str.split(rowdata.get(i));
		return split[0].substring(split[0].length()-3, split[0].length());
