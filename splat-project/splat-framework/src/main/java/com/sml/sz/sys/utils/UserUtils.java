...
import com.sml.sz.sys.security.SystemAuthorizingRealm.Principal;
...
public class UserUtils {
	...
	public static List<Role> getRoleList(Role role){
		@SuppressWarnings("unchecked")
		List<Role> roleList = null;//(List<Role>)getCache(CACHE_ROLE_LIST_COMPANY);
		User user = getUser();
		if(roleList == null){
			if ("A".equals(user.getPlatType())){
				roleList = roleDao.findAllList(role);
			}else{
				//role.getSqlMap().put("dsf", BaseService.dataScopeFilter(user.getCurrentUser(), "o", "u"));
				role.setOffice(user.getCompany());//获取当前用户所在机构的角色列表
				roleList = roleDao.findList(role);
			}
			putCache(CACHE_ROLE_LIST_COMPANY, roleList);
		}
		return roleList;
	}
	...
	public static List<Office> getOfficeList(){
		...
		if (officeList == null){
			User user = getUser();
			if (user.isAdmin()){//为管理员时查询所有
				officeList = officeDao.findAllList(new Office());
			}else{
				Office office = new Office();
				office.setParent(user.getCompany());//设置当前用户能拿到的机构权限
				//office.getSqlMap().put("dsf", BaseService.dataScopeFilter(user, "a", ""));//设置数据范围sql
